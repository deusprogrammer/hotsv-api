...
export class CommandResult {
    ...
    withEvent = (subject, action, adjustmentKey, adjustmentValue) => {
        this.adjustments.push(new StatAdjustment(subject, action, adjustmentKey, adjustmentValue));
        return this;
    }

    withAdjustment = (subject, adjustmentKey, adjustmentValue) => {
        return this.withEvent(subject, ADJUSTMENT, adjustmentKey, adjustmentValue);
    }
...
const distributeLoot = (monster, context) => {
    let lootDrops = [];
    let monsterDrops = [...context.monsterTable[monster.id].drops.map((drop) => {return {...drop}})];
    let taken = {};
 ...
 */
const hurt = (attackerName, defenderName, ability, context, isTrigger = false, performTriggers = true) => {
    ...
    let damageRoll = rollDice(ability.dmg, defender[ability.dmgStat.toLowerCase()]);
    ...
    if (!defender.isMonster && hit) {
        ...
        if (ability.dmgStat.toLowerCase() === "hp") {
            commandResult.withAdjustment(defenderName, ability.dmgStat.toLowerCase(), defenderAdjustments.hp);
        }
    }
    ...
    if (hit) {
        ...
        if (resistance > 1) {
            commandResult.withMessage(`${defender.name} is weak to ${ability.element.toLowerCase()}`);
        } else if (resistance < 1) {
            commandResult.withMessage(`${defender.name} is resistant to ${ability.element.toLowerCase()}`);
        }
    } else {
        commandResult.withMessage(`${attacker.name} attacked ${defender.name} and missed.`);
    }
    ...
    if (dead) {
        ...
        commandResult.withMessage(`${defender.name} was slain by ${attacker.name}.`);
    }
    ...
    if (hit && !dead && performTriggers) {
        for (const trigger of ability.triggers) {
            ...
            if (triggerRoll <= trigger.chance) {
                commandResult.withMessage(`${attacker.name}'s ${attacker.equipment.hand.name}'s ${trigger.ability.name} activated!`);
                switch (ability.element) {
                    case "HEALING":
                        results = heal(attackerName, attackerName, ability, context);
                        break;
                    case "BUFFING":
                        results = heal(attackerName, attackerName, ability, context);
                        break;
                    default:
                        results = hurt(attackerName, defenderName, ability, context, true, true);
                        break;
                }
...
const buff = (attackerName, defenderName, ability, context) => {
    ...
    commandResult.withMessage(`${defender.name} is affected by ${ability.name}`);
...
const cleanse = (attackerName, defenderName, ability, context) => {
    ...
    tokens.forEach((token) => {
        ...
        commandResult.withMessage(`${defender.name} is cured of ${effectToRemove}`);
...
const heal = (attackerName, defenderName, ability, context) => {
    ...
    commandResult.withAdjustment(attackerName, AP, -ability.ap);
    commandResult.withAdjustment(defenderName, ability.dmgStat.toLowerCase(), healingAmount);
    commandResult.withMessage(`${ability.name} healed ${defender.name} for ${healingAmount} ${ability.dmgStat}`);
...
export const attack = (attackerName, defenderName, context) => {
    ...
    let result = hurt(attackerName, defenderName, {
        name: "attack",
        dmg: weapon.dmg,
...
export const useAbility = (attackerName, defenderName, ability, context) => {
    ...
    for (let i in abilityTargets) {
        ...
        if (ability.element === "HEALING") {
            ...
            results.push(cleanse(attackerName, abilityTarget, ability, context));
        } else {
